...
export const config = {
  ...
  stockfishRequired: process.env.STOCKFISH_REQUIRED !== 'false',
  stockfishPath: process.env.STOCKFISH_PATH || 'stockfish',
  stockfishDepth: Number(process.env.STOCKFISH_DEPTH || 8),
  stockfishMovetimeMs: Number(process.env.STOCKFISH_MOVETIME_MS || 0),
  stockfishTimeoutMs: Number(process.env.STOCKFISH_TIMEOUT_MS || 2500),
  stockfishAdjudicationThresholdCp: Number(process.env.STOCKFISH_ADJUDICATION_THRESHOLD_CP || 150),
